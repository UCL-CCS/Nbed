...
def huzinaga_HF(
    ...
    logger.debug(f"{dm_enviroment.shape=}")
    if dm_enviroment.ndim == 3:
    ...
    if dm_initial_guess is None:
        ...
        logger.debug(f"{scf_method.get_hcore().shape=}")
        ...
        if fock.ndim == 3:
            ...
            huzinaga_op_std = np.array(
                [
                    -(fds_alpha + fds_alpha.swapaxes(-1, -2)),
                    -(fds_beta + fds_beta.swapaxes(-1, -2)),
                ]
            )
        else:
            fds = fock @ dm_env_S
            huzinaga_op_std = -0.5 * (fds + fds.swapaxes(-1, -2))
