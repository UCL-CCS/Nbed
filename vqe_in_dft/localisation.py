...
def localize_molecular_orbs(
    pyscf_scf: gto.Mole,
    n_active_atoms: int,
    localization_method: str,
    occ_THRESHOLD: float = 0.95,
    virt_THRESHOLD: float = 0.95,
    sanity_check: bool = False,
    run_virtual_localization: bool = False,
) -> Tuple[
    np.array,
    np.array,
    np.array,
    ...
    Function returns active and environment systems.
    ...
    if localization_method.lower() == "spade":
        (
            ...
            active_MO_inds,
            enviro_MO_inds,
        ) = spade(pyscf_scf, n_active_atoms)
    else:
    ...
    if run_virtual_localization is True:
        (
            ...
            pyscf_scf, n_active_atoms, virt_THRESHOLD=virt_THRESHOLD
        )
...
def localize_virtual_orbs(
    pyscf_scf: StreamObject, n_active_atoms: int, virt_THRESHOLD: float = 0.95
...
def orb_change_basis_operator(
    pyscf_scf: StreamObject, c_all_localized_and_virt: np.array, sanity_check=False
) -> np.ndarray:
